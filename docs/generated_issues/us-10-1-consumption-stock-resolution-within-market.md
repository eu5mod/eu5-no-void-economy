...
- callers use `cbp_resolve_stock_consumption` with `consumer_country`,
...
- the implementation removes stock through `cbp_remove_stock` only;
...
| Central removal | ModeU5 | `cbp_remove_stock` | CONFIRMED | 075 |
...
```txt
Depends on: US-10.0, cbp_remove_stock, demand exposure/fallback
...
- Remove through `cbp_remove_stock` only.
...
- [ ] Every mutation uses `cbp_remove_stock`.
