...
US-20 extends route economics by assigning separate meanings:
...
The old vanilla price-side buying/selling bonus must be removed so that the #120 buy/sell model does not stack on top of vanilla.

## Money-side rule

Old price-side bonus to remove:
...
Until those surfaces are confirmed in TECH-01, only the ModeU5 accumulator may be updated; vanilla income/profit must remain blocked.
...
Therefore the previous `remove_good` probe is no longer required. The implementation tracks this as market-stockpile loss application rather than a blocked missing surface.
...
Destination promotion decides whether a detailed receiver country can exist. Origin promotion decides whether the base receipt path is an add-at-destination or a country-market transfer.
