...
| Rebalance Economy | Optional; included in the recommended playset | US-04, US-04-UI, US-05, US-05-UI, US-08, US-08-UI, US-09, US-09-UI, plus the US-07 trade-building fields composed into the same generated complete building files |
...
Publication starts by deleting and recreating all five managed package roots.
This clean-install contract prevents renamed or removed files from surviving
beside their replacements. The installer also removes the known legacy
`eu5voideco` monolith, then verifies each deployed tree is an exact content
mirror of its source payload. It never deletes the parent mod directory or
unrelated mods.
...
- the composed US-07 trade-building estate-power reduction carried by the generated `cbp_trade_buildings.txt` replacement objects is also absent;
...
- when Rebalance Economy is present, the overlapping US-07 `local_burghers_estate_power` trade-building reduction is composed into the same per-building generation plan; it is materialized in the structural `REPLACE` object when one is required, otherwise as part of the building's single sparse `INJECT` delta;
