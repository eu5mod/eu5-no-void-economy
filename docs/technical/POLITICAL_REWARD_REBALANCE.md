...
The same generated `default_values.txt` override raises the five production
method debug profit-margin targets by 10 percent:

```txt
rural_profit_margin       0.20 -> 0.22
guild_profit_margin       0.20 -> 0.22
workshop_profit_margin    0.25 -> 0.275
manufactory_profit_margin 0.30 -> 0.33
mills_profit_margin       0.35 -> 0.385
```

Every transformed scalar generated by this feature records its source as
`# VANILLA = <value>`. Dynamic value blocks retain their multiplier instead,
because they do not have one scalar vanilla value.
...
upkeep through the supported `BUILDING_UPKEEP_FACTOR` define. Do not restore
the removed `building_upkeep_costs` auto modifier: EU5 1.3 rejects that
modifier type.
