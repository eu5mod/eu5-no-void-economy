...
| 007 | Core | Store durable multidimensional ModeU5 state | scoped owner plus one scoped map key | `add_to_variable_map`, <code>variable_map(name&#124;key)</code>, `remove_from_variable_map`, `clear_variable_map` | variable map | Wiki: Variable; Effect | CONFIRMED | N/A | Follow `VARIABLE_MAP_STORAGE_MODEL.md`. Maps provide one keyed dimension on an owner: use country-scoped per-good maps keyed by market for country × market × good, market-scoped maps keyed by goods for market × good, and location-scoped maps keyed by goods for location × good. Map names are static; update existing keys by remove/re-add. |
...
| 015 | US-01 | Current country stock in market for good | country × market × good | country-scoped `modeu5_<good>_stock_by_market` map keyed by market | internal variable map | ModeU5; TECH-01 007 | CONFIRMED | N/A | Logical name remains `country_market_good_stock`. Only centralized stock effects may write these maps. Missing entries default explicitly to zero. |
...
| 017 | US-01 | Country stock capacity | country × market × good | country-scoped `modeu5_<good>_stock_cap_by_market` map keyed by market | internal variable map | ModeU5; TECH-01 007 | CONFIRMED | N/A | Logical name remains `country_market_good_stock_cap`. Missing entries default explicitly to zero. |
...
| 025 | US-00.1 | Store and read a monthly ledger keyed by country × market × good | country-scoped per-good maps keyed by market | `modeu5_<good>_produced_by_market`, `modeu5_<good>_added_by_market`, `modeu5_<good>_rejected_by_market` plus map add/read/remove/clear operations | variable storage and keying | Wiki: Variable; Effect; TECH-01 007 | CONFIRMED | N/A | Build and update the ledger monthly through the centralized helper. Existing keys require remove/re-add; clear only after all monthly consumers run. Do not reconstruct it from final stock snapshots. |
...
| 040 | US-04 | Track yearly satisfied/unsatisfied months | location × good | location-scoped `modeu5_pop_demand_satisfied_months` and `modeu5_pop_demand_unsatisfied_months` maps keyed by goods scope | internal variable map | ModeU5; TECH-01 007 | CONFIRMED | N/A | Missing entries default to zero. Read, adjust, remove, and re-add each key; reset only after US-04 yearly adaptation reads it. |
...
| 077 | US-10.3 | Track satisfied and unsatisfied demand outcomes | consumer target × good, optionally country × market × good | location-scoped outcome maps keyed by goods for Pops; country-scoped per-good maps keyed by market for broader aggregates | internal variable map | ModeU5; TECH-01 007 | CONFIRMED | N/A | One request's arithmetic stays local. Persist only monthly/yearly aggregates. Pop outcome maps feed US-04 and share its location × good key contract. |
