...
| 007 | Core | Store one durable field indexed by a scoped key | scoped owner plus one scoped map key | `add_to_variable_map`, <code>variable_map(name&#124;key)</code>, `remove_from_variable_map`, `clear_variable_map` | variable map | Wiki: Variable 1.1 variable maps; Effect | CONFIRMED | N/A | A native map entry is one key mapped to one number or scope. Follow `VARIABLE_MAP_STORAGE_MODEL.md`: represent a multi-field logical record through a synchronized map family until 088 is resolved. Map names are static; update existing keys by remove/re-add. |
...
| 016 | CORE-01 / US-01 | Market stock for good | market × good | market-scoped `modeu5_market_good_stock` map keyed by goods scope | internal variable map | ModeU5; TECH-01 007 | CONFIRMED | N/A | Logical name remains `market_good_stock`. This is an aggregate/cache rebuilt only from country stock maps. |
...
| 018 | CORE-01.1 / CORE-01.3 / US-01 | Available stock capacity | ModeU5 | stock_cap - stock | internal scripted value | ModeU5 | CONFIRMED | N/A | Defined by mod. |
