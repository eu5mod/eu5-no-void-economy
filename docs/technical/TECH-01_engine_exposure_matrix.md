...
- Rows 044 and 081 are ModeU5 design/scope contracts. Their `CONFIRMED` status does not claim an undocumented vanilla accounting endpoint.
...
| 021 | US-00.1 | Read production quantity at its source for the country-rooted ledger | current country × production source × location × good | `goods_output`, `raw_material_output`, `produced_in_country`, `produced_in_market` | value / production integration output | Wiki: Scope link; Trigger; Building modding | NOT_CONFIRMED | One explicit source-level production estimate if no reliable value is exposed | Location totals and country/market aggregates are documented, but no exact building/RGO source quantity is documented. Country attribution is handled separately by 081. |
...
| 043 | US-05 | Country wealth | country | country `wealth` / location wealth aggregate | value | Wiki pages | NOT_CONFIRMED | ModeU5 reconstructed/estimated wealth | `wealth_impact_threshold` is a static goods-demand field, not a readable country-wealth value. |
| 044 | US-05 | Define and use the ModeU5 Economic Base | country/scripted value and ModeU5-controlled call sites | `modeu5_slider_cost_base = Wealth + monthly_trade_income` | internal scripted value / design contract | ModeU5 specification; AGENTS.md | CONFIRMED | N/A | All ModeU5-controlled Stability and legitimacy-producing Court calculations must use this value. This does not claim that every vanilla slider call site can be replaced; 041 and 045 cover optional reconciliation needs. |
...
| 056 | US-10.2 | Read vanilla per-trade requested quantity or used capacity | trade | per-trade quantity / `used_trade_capacity` | value | Wiki: Trigger; Scope link | NOT_CONFIRMED | Use US-10.2 `transferred_quantity` for ModeU5 transfers | Market-level `used_merchant_capacity`, `merchant_capacity`, and trade `capacity_market` exist, but no per-trade quantity/capacity value is documented. |
...
| 081 | US-00.1 | Establish the ModeU5 ledger country for each production source | country-rooted cycle; building/location → country | current country from `monthly_country_pulse`; building/location `owner` | scope contract / scope link | Wiki: On actions; Scope link; Building modding; ModeU5 | CONFIRMED | N/A | The ledger runs in country scope. Attribute building sources through building `owner` and RGO/location sources through location `owner`, validating against the current country. This is a ModeU5 attribution rule, not proof of vanilla income-credit semantics. |
...
## Non-confirmed exposure impact matrix

This matrix maps each `NOT_CONFIRMED` engine exposure to the user stories that reference it directly or inherit its blocked behavior.

| TECH-01 | Non-confirmed exposure | Impacted US | Blocked behavior | Current fallback / boundary |
|---:|---|---|---|---|
| 021 | Source-level building/RGO production quantity | US-00.1, EPIC US-00, US-00-UI, US-01 | Measure production before stock insertion at the required source granularity | Use one explicit source-level production estimate if accepted. |
...
| 041 | Read current Stability/Court slider cost | US-05, US-05-UI | Compare vanilla cost with the ModeU5 target when optional reconciliation is needed | Keep reconciliation debug-only until a readable value is confirmed. |
| 043 | Read country wealth | US-05, US-05-UI | Calculate the required `Wealth + Trade Income` ModeU5 Economic Base | Use one accepted ModeU5 reconstructed/estimated wealth value. |
| 045 | Present slider reconciliation visibly | US-05, US-05-UI | Expose any optional vanilla slider correction to the player | Use a debug report if no safe visible gameplay effect is confirmed. |
| 056 | Read per-trade quantity or used capacity | US-10.2, EPIC US-10 | Resolve vanilla requested quantity and calculate cost on actual trade quantity | Use US-10.2 `transferred_quantity` for ModeU5 transfers. |
| 078 | Reliably classify a country as horde/non-horde | US-13 | Select conquest-cost variants by attacker government class | Keep US-13 excluded until confirmed. |
