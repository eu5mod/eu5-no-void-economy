...
| 154 | US-177 | Identify food goods and reduce their food contribution independently | goods database object | numeric `food` field in `in_game/common/goods`; food good iff `food > 0`; generated value = vanilla / configured divisor, default `3` | static field / generated exact-path override and manifest | Local vanilla `goods/readme.txt`; supported vanilla goods files; `us177_food_goods_manifest.json` | CONFIRMED | Generate exact-path goods overrides from vanilla | The supported reference contains 14 food goods, currently in `03_food.txt`. The generator changes only positive `food` assignments and preserves unrelated fields. |
