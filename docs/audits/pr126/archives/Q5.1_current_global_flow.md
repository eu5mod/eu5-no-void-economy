...
monthly_country_pulse(country)
  -> cbp_run_monthly_stock_cycle
      -> cbp_stock_runtime_ready_trigger
...
flowchart TB
    ...
    subgraph LOOP_COUNTRY["Loop: monthly_country_pulse / current country"]
        direction TB
        A["monthly_country_pulse<br/>engine country iteration"] --> B["cbp_run_monthly_stock_cycle"]
        B --> C{"runtime ready?"}
        ...
        subgraph LOOP_CAP_MARKET["Loop: every_market_present_in_country<br/>current-country capacity split"]
            direction TB
            P1 --> CAP0["cbp_recalculate_saved_country_storage_capacities"]
        ...
        subgraph LOOP_CAP_POOL["Dependency loop: every_owned_location<br/>country location-capacity pool"]
            direction TB
            CAPD1["cbp_rebuild_country_location_capacity_pool<br/>(outside hot monthly split)"] --> CAPD2["sum owned-location rank capacity"]
        ...
        P4 --> L0["cbp_run_monthly_promoted_market_local_cycle"]
        ...
        subgraph LOOP_MARKET_CENTER["Loop: every_market_center_in_country<br/>market-center-owned local branch"]
            ...
            L4 -->|detailed| L5["cbp_run_promoted_market_live_local_branch_market_all_goods"]
            ...
            subgraph LOOP_MARKET_LOC["Loop: every_location_in_market<br/>rebuild countries_present_in_market"]
                ...
                M2 --> M3["dedupe into cbp_countries_present_in_market"]
            end

            subgraph LOOP_COUNTRIES_CAP_US00["Loop: every_in_global_list cbp_countries_present_in_market<br/>Q4.1 fused capacity + US-00 pass"]
                direction TB
                M3 --> D1["for each present country"]
                D1 --> D2["refresh country-market capacity"]
                D2 --> U1["cbp_pr71_process_us00_monthly_market_active_goods"]

                subgraph LOOP_US00_GOODS["Generated literal per-good call surface<br/>US-00 active-good guards"]
                    direction TB
                    U1 --> U2["cbp_pr71_process_us00_monthly_market_good_GOOD"]
                    U2 --> U3{"produced_in_market > 0<br/>or, if not produced,<br/>previous US-00 state?"}
                    U3 -->|yes| U4["cbp_process_us00_monthly_market_good_GOOD"]
            ...
            subgraph LOOP_COUNTRIES_US10["Loop: every_in_global_list cbp_countries_present_in_market<br/>US-10 pending-request pass"]
                direction TB
                U7 --> S0["for each present country"]
                S0 --> S1["cbp_pr71_process_us10_monthly_market_pending_goods"]

                subgraph LOOP_US10_GOODS["Generated literal per-good call surface<br/>US-10 pending-request guards"]
                    direction TB
                    S1 --> S2["cbp_pr71_process_us10_monthly_market_good_GOOD"]
                    S2 --> S3{"pending same-market request?"}
                    S3 -->|yes| S4["cbp_process_us10_monthly_market_good_GOOD"]
