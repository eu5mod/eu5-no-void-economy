...
```mermaid
flowchart TB
    %% Vertical-first layout: keep loop subgraphs in one top-to-bottom chain.
    ...
    subgraph LOOP_CAP_POOL["Dependency loop: every_owned_location<br/>country location-capacity pool"]
        ...
        CAPD2 --> CAPD0["country location-capacity pool"]
    end

    P1 -. reads cached pool .-> CAPD0
    CAPD0 -. feeds .-> CAP0

    subgraph LOOP_CAP_MARKET["Loop: every_market_present_in_country<br/>current-country capacity split"]
        direction TB
        CAP0["modeu5_recalculate_saved_country_storage_capacities"] --> CAP1["count current-country markets"]
    ...
    subgraph LOOP_MARKET_CENTER["Loop: every_market_center_in_country<br/>market-center-owned local branch"]
        ...
        L4 -. fallback or blocked .-> F0["record fallback/blocked<br/>no ModeU5 stock mutation"]
