...
```mermaid
flowchart TD
    A[monthly_country_pulse country] --> B[modeu5_run_monthly_stock_cycle]
    B --> C{runtime ready?}
    ...
    L0 --> L1[every_market_center_in_country]
    L1 --> L2[mark monthly market seen]
    L2 --> L3[prepare runtime accounting mode]
    L3 --> L4{market runtime mode}
    L4 -->|detailed| L5[rebuild countries_present_in_market once]
    L5 --> L6[refresh capacities for countries present]
    L6 --> L7[US-00 all present countries / generated goods]
    L7 --> L8[US-10 same-market requests all present countries / generated goods]
    L8 --> L9[record local market processed]
    ...
    F1 --> T0
    F2 --> T0
    T0 --> T1[country-owned every_trade pass]
    T1 --> T2[inter-market trade handlers]
