...
```mermaid
flowchart TD
    A[monthly_country_pulse<br/>outer country loop] --> B[modeu5_run_monthly_stock_cycle]
    B --> C{runtime ready?}
    ...
    subgraph CAP[Current-country capacity refresh loops]
        P1 --> CAP0[modeu5_recalculate_saved_country_storage_capacities]
        CAP0 --> CAP1[capacity pool<br/>every_owned_location]
        CAP0 --> CAP2[market split<br/>every_market_present_in_country]
        CAP2 --> CAP3[store country-market capacity]
    end

    P4 --> L0[modeu5_run_monthly_promoted_market_local_cycle]
    L0 --> L1[market-owner loop<br/>every_market_center_in_country]
    ...
    L5 --> M0[rebuild countries_present_in_market once]
    M0 --> M1[market location scan<br/>every_location_in_market]
    M1 --> M2[location owner scope<br/>owner link to country]
    M2 --> M3[dedupe work cache<br/>modeu5_countries_present_in_market]

    M3 --> D1[countries-present loop<br/>every_in_global_list capacity]
    D1 --> D2[refresh country-market capacity]
    M3 --> D3[countries-present loop<br/>every_in_global_list US-00]
    D3 --> D4[generated goods adapter loop<br/>modeu5_process_us00_monthly_market_all_goods]
    D4 --> D5[US-00 all present countries before consumption]
    M3 --> D6[countries-present loop<br/>every_in_global_list US-10]
    D6 --> D7[generated goods adapter loop<br/>modeu5_process_us10_monthly_market_all_goods]
    D7 --> D8[US-10 same-market requests]
    D8 --> L9[record local market processed]

    L4 -->|vanilla fallback| F1[record fallback / no ModeU5 stock mutation]
    L4 -->|blocked| F2[record blocked]

    L9 --> T0[modeu5_run_monthly_country_trade_owner_cycle]
    F1 --> T0
    F2 --> T0
    T0 --> T1[country-scope trade loop<br/>every_trade]
