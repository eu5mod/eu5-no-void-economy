...
ModeU5 US-04 RESULT pop_demand_adaptation BLOCKED reason=location_estate_demand_exposure_not_confirmed
...
mutation are blocked until TECH-01 149 confirms a direct live
`location × estate × good` demand read or TECH-01 150 confirms the local
`modeu5_us04_reconciliation_coefficient × proxy_estate_size_at_location`
calculation. The intended future production path is a
pre-US-10 additional-demand preparation pass, not a post-US-10 correction.
Current tests expect `location_estate_demand_exposure_not_confirmed`, no stock
removal, no market delta, and no estate charge. There is no `peasants_estate`
fallback.
...
ModeU5 US-04 ESTATE ACCOUNTING location_level=1 goods=2 estates=4 ... result=blocked reason=location_estate_demand_exposure_not_confirmed
ModeU5 TEST BLOCKED scenario=us04_estate_level_accounting reason=location_estate_demand_exposure_not_confirmed
```
