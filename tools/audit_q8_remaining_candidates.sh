...
probe_effects="packages/cbp_core_tests/in_game/common/scripted_effects/cbp_q8_probe_effects.txt"
probe_events="packages/cbp_core_tests/in_game/events/cbp_q8_probe_debug_events.txt"
...
require_match '^cbp_debug_run_q8_remaining_candidate_probes[[:space:]]*=' "$probe_effects" 'aggregate Q8 probe entry point exists'
require_match '^cbp_q8_probe_debug\.1[[:space:]]*=' "$probe_events" 'Q8 probe event menu exists'

require_match '^cbp_q8_probe_us10_pending_gate[[:space:]]*=' "$probe_effects" 'Q8.2 runtime probe exists'
...
require_match '^cbp_q8_probe_helper_inventory[[:space:]]*=' "$probe_effects" 'Q8.4 runtime/static bridge probe exists'
require_match 'test_cbp_q8_4_helper_inventory_probe_passed' "$probe_effects" 'Q8.4 probe reports PASS flag'

require_match '^cbp_q8_probe_dirty_cache_lifecycle[[:space:]]*=' "$probe_effects" 'Q8.5 dirty-cache lifecycle probe exists'
require_match 'cbp_mark_market_country_cache_dirty' "$probe_effects" 'Q8.5 probe marks a market dirty'
require_match 'cbp_repair_dirty_market_country_caches' "$probe_effects" 'Q8.5 probe repairs dirty market-country cache'

require_match '^cbp_q8_probe_market_sliced_verifier_candidate[[:space:]]*=' "$probe_effects" 'Q8.6 market-slice candidate probe exists'
require_match 'test_cbp_q8_probe_market_slice_candidates' "$probe_effects" 'Q8.6 probe uses a candidate-market list'

require_match '^cbp_q8_probe_global_market_iterator_exposure[[:space:]]*=' "$probe_effects" 'Q8.7 global-market exposure probe exists'
