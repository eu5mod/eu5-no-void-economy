...
def validate_us17_us20_static_contract(
    *,
    ...
    expect("modeu5_trade_rework_enabled_trigger = yes" in route_effect, "US-17/US-20 route effect must defensively gate itself")
    expect("modeu5_prepare_trade_efficiency_reconciliation_runtime_metrics_once = yes" in route_effect, "US-17/US-20 route effect must prepare metrics inside the gated body")
    ...
    expect(
        "every_market_center_in_country = {" not in country_trade_owner_effects + q8_7_global_owner_effects + trade_reconciliation_effects,
    ...
    expect(
        unsafe_global_counter.search(trade_reconciliation_effects) is None,
