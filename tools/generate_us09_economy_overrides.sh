...
has_us09_building_target_field() {
...
while IFS= read -r -d '' source_file; do
	if ! has_us09_building_target_field "$source_file"; then
		continue
	fi
...

	{
		...
		strip_utf8_bom_stream "$source_file" \
			| MODEU5_US09_OUTPUT_MULTIPLIER="$output_multiplier" \
				MODEU5_US09_SOURCE_BASENAME="$source_basename" \
				MODEU5_US07_TRADE_BURGHERS_ESTATE_POWER_MULTIPLIER="$us07_trade_burghers_estate_power_multiplier" \
				perl -pe '
			BEGIN {
				$multiplier = $ENV{"MODEU5_US09_OUTPUT_MULTIPLIER"};
				$source_basename = $ENV{"MODEU5_US09_SOURCE_BASENAME"};
				$us07_burghers_estate_power_multiplier = $ENV{"MODEU5_US07_TRADE_BURGHERS_ESTATE_POWER_MULTIPLIER"};
			}
			...
			if (/^(\s*(?:output|local_trades_per_burgher|local_merchant_capacity|merchant_capacity_from_building)\s*=\s*)([0-9]+(?:\.[0-9]+)?)(\s*(?:#.*)?)$/) {
				$_ = $1 . format_decimal($2 * $multiplier) . $3 . "\n";
			}

			if ($source_basename eq "trade_buildings.txt" && /^(\s*local_burghers_estate_power\s*=\s*)([0-9]+(?:\.[0-9]+)?)(\s*(?:#.*)?)$/) {
				$_ = $1 . format_decimal($2 * $us07_burghers_estate_power_multiplier) . $3 . "\n";
			}
		'
	} > "$output_file"
...
# EU5 vanilla has changed this field name across builds / documentation.
# Treat these as the same target surface while staying inside the selected
# top-level Pop-type block.
...
promotion_line = re.compile(
    r"^(\s*(?:" + "|".join(re.escape(name) for name in promotion_field_names) + r")\s*=\s*)"
    r"(-?[0-9]+(?:\.[0-9]+)?)(\s*(?:#.*)?)$"
)
...
for source_file in sorted(source_dir.glob("*.txt")):
    ...
    for line in lines:
        ...
        if current_target in requested:
            promotion_match = promotion_line.match(line)
            if promotion_match:
                old_value = float(promotion_match.group(2))
                new_value = old_value * multiplier[current_target]
                line = f"{promotion_match.group(1)}{format_decimal(new_value)}{promotion_match.group(3)}"
                file_changed = True
...
if missing:
    ...
    accepted_fields = ", ".join(promotion_field_names)
    raise SystemExit(
        "Could not generate US-09 pop-type promotion overrides for "
        f"{missing_list}; seen target blocks with no recognized promotion field: {seen_list}. "
