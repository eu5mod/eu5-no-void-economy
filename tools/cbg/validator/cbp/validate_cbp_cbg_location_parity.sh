...
./tools/generate_cbp_location_overrides.sh \
	...
	--output-file "$work_dir/reference/main_menu/common/static_modifiers/cbp_location.txt"
...
reference="$work_dir/reference/main_menu/common/static_modifiers/cbp_location.txt"
if ! cmp -s "$reference" "$candidate"; then
	diff -u "$reference" "$candidate" || true
	exit 1
fi
printf '%s\n' 'CBP/CBG location parity passed: dedicated REPLACE output is byte-identical.'
