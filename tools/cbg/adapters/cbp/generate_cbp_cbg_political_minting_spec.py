#!/usr/bin/env python3
"""Compile #188 political and minting edge cases into one composed CBG spec."""

from __future__ import annotations

import argparse
import json
import shutil
import subprocess
import sys
import tempfile
from pathlib import Path

REPO_ROOT = Path(__file__).resolve().parents[4]
if str(REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))

from tools.cbg.community_balance_generator import scan_objects

EXACT_PATH_COMMON_DATABASES = {"gods"}


def run(command: list[str]) -> None:
    subprocess.run(command, check=True, stdout=subprocess.DEVNULL)


def cbp_prefixed(relative: str) -> str:
    path = Path(relative)
    return (path.parent / f"cbp_{path.name}").as_posix()


def common_database(relative: str) -> str | None:
    parts = Path(relative).parts
    try:
        common_index = parts.index("common")
    except ValueError:
        return None
    if common_index + 1 >= len(parts):
        return None
    return parts[common_index + 1]


def changed_top_level_objects(
    source: Path,
    transformed: Path,
) -> list[tuple[str, list[str]]]:
    def normalized(lines: list[str]) -> list[str]:
        result = [line.rstrip() for line in lines]
        while result and not result[-1]:
            result.pop()
        return result

    source_lines = source.read_text(encoding="utf-8-sig").splitlines()
    transformed_lines = transformed.read_text(encoding="utf-8-sig").splitlines()
    source_objects = {
        obj.path[0]: obj for obj in scan_objects(source_lines) if len(obj.path) == 1
    }
    transformed_objects = {
        obj.path[0]: obj
        for obj in scan_objects(transformed_lines)
        if len(obj.path) == 1
    }
    if source_objects.keys() != transformed_objects.keys():
        raise ValueError(
            f"Common-database transformation changed the top-level object set: "
            f"{source}"
        )

    changed: list[tuple[str, list[str]]] = []
    reconstructed = list(source_lines)
    replacements: list[tuple[int, int, list[str]]] = []
    for name, source_obj in source_objects.items():
        transformed_obj = transformed_objects[name]
        source_block = source_lines[source_obj.start : source_obj.end + 1]
        transformed_block = transformed_lines[
            transformed_obj.start : transformed_obj.end + 1
        ]
        if normalized(source_block) == normalized(transformed_block):
            continue
        changed.append((name, transformed_block))
        replacements.append((source_obj.start, source_obj.end, transformed_block))

    for start, end, replacement in sorted(replacements, reverse=True):
        reconstructed[start : end + 1] = replacement
    if normalized(reconstructed) != normalized(transformed_lines):
        raise ValueError(
            f"Common-database transformation changed content outside top-level "
            f"objects: {source}"
        )
    if not changed:
        raise ValueError(f"Staged transformed file has no effective object change: {source}")
    return changed


def build_spec(args: argparse.Namespace) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="cbp-cbg-political-") as temporary:
        staged = Path(temporary) / "package"
        run([
            "python3", str(args.repo_root / "tools/generate_us177_minting_overrides.py"),
            "--game-root", str(args.game_root), "--package-root", str(staged),
            "--multiplier", args.minting_multiplier,
        ])
        run([
            "python3", str(args.repo_root / "tools/generate_political_reward_overrides.py"),
            "--game-root", str(args.game_root), "--package-root", str(staged),
            "--skip-central-default-values",
        ])
        minting = json.loads(
            (staged / "cbp_generated/us177_minting_income_manifest.json").read_text()
        )
        political = json.loads(
            (staged / "cbp_generated/political_reward_overrides_manifest.json").read_text()
        )
        if args.minting_discovery_manifest:
            args.minting_discovery_manifest.parent.mkdir(parents=True, exist_ok=True)
            shutil.copyfile(
                staged / "cbp_generated/us177_minting_income_manifest.json",
                args.minting_discovery_manifest,
            )
        if args.political_discovery_manifest:
            args.political_discovery_manifest.parent.mkdir(parents=True, exist_ok=True)
            shutil.copyfile(
                staged / "cbp_generated/political_reward_overrides_manifest.json",
                args.political_discovery_manifest,
            )
        paths = {entry["path"] for entry in minting["files"]}
        paths.update(
            entry["path"] if entry.get("package_relative") else f"in_game/{entry['path']}"
            for entry in political["files"]
        )
        transformations: list[dict[str, object]] = []
        owned_outputs: set[str] = set()
        for relative in sorted(paths):
            output = staged / relative
            source = args.game_root / relative
            if not output.is_file() or not source.is_file():
                raise ValueError(f"Composed CBG staging/source missing for {relative}")
            database = common_database(relative)
            if database in EXACT_PATH_COMMON_DATABASES:
                for object_name, replacement in changed_top_level_objects(
                    source, output
                ):
                    transformations.append({
                        "file": relative,
                        "object": object_name,
                        "field": "__object__",
                        "operation": "replace_object",
                        "value": replacement,
                        "provenance": "preserve",
                        "render_mode": "normalized",
                    })
                owned_outputs.add(relative)
            elif database is not None:
                header = [
                    "# Generated by the CBP political/minting CBG adapter.",
                    "# Do not edit manually.",
                    f"# Vanilla source: {relative}",
                ]
                for object_name, replacement in changed_top_level_objects(
                    source, output
                ):
                    transformations.append({
                        "file": relative,
                        "object": object_name,
                        "field": "__object__",
                        "operation": "replace_object",
                        "value": replacement,
                        "provenance": "preserve",
                        "render_mode": "replace_objects",
                        "header": header,
                    })
                owned_outputs.add(cbp_prefixed(relative))
            else:
                transformations.append({
                    "file": relative,
                    "object": "",
                    "field": "__file__",
                    "operation": "replace_file",
                    "value": output.read_text(encoding="utf-8"),
                    "provenance": "preserve",
                    "render_mode": "provided",
                })
                owned_outputs.add(relative)
        return {
            "schema_version": 1,
            "mod_id": "cbp-economy-rebalance-political-minting",
            "business_rule": "Scale political rewards and research modifiers, then compose minting changes into the same Vanilla-derived files.",
            "transformations": transformations,
            "scope_contract": {
                "owned_outputs": sorted(owned_outputs),
                "phase": "political-and-minting-composition",
                "packaging": (
                    "CBP-prefixed REPLACE objects for flat common databases; "
                    "exact-path files for common databases with nested global "
                    "registries and for event databases"
                ),
                "edge_case_compilers": [
                    "generate_us177_minting_overrides.py",
                    "generate_political_reward_overrides.py",
                ],
                "central_default_values": "delegated-to-focused-cbg-family",
                "building_types": "delegated-to-focused-cbg-building-family",
            },
        }


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--game-root", type=Path, required=True)
    parser.add_argument("--repo-root", type=Path, default=Path(__file__).resolve().parents[4])
    parser.add_argument("--minting-multiplier", required=True)
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--minting-discovery-manifest", type=Path)
    parser.add_argument("--political-discovery-manifest", type=Path)
    args = parser.parse_args()
    payload = build_spec(args)
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(
        f"Generated {args.output} with {len(payload['transformations'])} "
        "composed political/minting outputs."
    )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
