...
Options:
  ...
  --rgo-price-percent N                  Independent inverse offset for RGO expansion prices
                                          (default: same as --percent for direct legacy calls)
  ...
  --skip-rgo-prices                      Delegate RGO price output to CBG
  --skip-pop-promotions                  Delegate Pop promotion output to CBG
  --skip-building-overrides              Keep #188 building analysis manifests but delegate runtime files to CBG
...
percent=""
rgo_price_percent="${MODEU5_US09_RGO_PRICE_OFFSET_PERCENT:-}"
...
skip_rgo_prices=0
skip_pop_promotions=0
skip_building_overrides=0
...
while (($# > 0)); do
	case "$1" in
		...
		--skip-rgo-prices)
			...
			;;
		--skip-pop-promotions)
			skip_pop_promotions=1
			shift
			;;
		--skip-building-overrides)
			skip_building_overrides=1
			shift
			;;
		--help)
			usage
...
if [[ -z "$rgo_price_percent" ]]; then
	rgo_price_percent="$percent"
fi
validate_percent "MODEU5_US09_RGO_PRICE_OFFSET_PERCENT" "$rgo_price_percent"
...
if [[ "$skip_pop_promotions" -eq 0 && -d "$pop_types_output_dir" ]]; then
...
rgo_price_multiplier="$(awk -v percent="$rgo_price_percent" 'BEGIN { printf "%.12f", 1 / (1 + (percent / 100)) }')"
...
if [[ "$skip_building_overrides" -eq 0 ]]; then
	while IFS= read -r -d '' generated_file; do
		if head -n 1 "$generated_file" | grep -qx '# Generated by tools/generate_us09_economy_overrides.sh.'; then
			rm -f "$generated_file"
		fi
...
if [[ "$promotion_overrides_enabled" -eq 1 && "$skip_pop_promotions" -eq 0 ]]; then
...
if [[ "$skip_building_overrides" -eq 1 ]]; then
	printf 'Analyzed %d building override files for CBG; generated 1 RGO expansion price override file and %d pop-type promotion override files for US-09 (output/RGO %s%%; trade capacity %s%%).\n' \
