...
def build_spec(game_root: Path, divisor: Decimal) -> dict[str, object]:
    ...
    for source_path, entries in sorted(grouped.items()):
        for entry in sorted(entries, key=lambda item: str(item["good"])):
            transformations.append({
                ...
                "header": [
                    "# Generated by the CBP US-177 food CBG adapter.",
                    "# Do not edit manually.",
                    f"# Vanilla source: {source_path}",
                ],
            })
    owned_outputs = [
        (Path(source).parent / f"cbp_{Path(source).name}").as_posix()
    ...
    return {
        ...
        "scope_contract": {
            ...
            "packaging": "CBP-prefixed REPLACE:<good> objects",
        },
    }
