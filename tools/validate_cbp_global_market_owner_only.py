...
def main() -> int:
    ...
    runtime_mermaid = mermaid_block(RUNTIME_FLOW_PATH.read_text(encoding="utf-8"))
    ...
    for forbidden in (
        ...
        require(
            forbidden not in live_effects,
            f"Retired owner path reintroduced in live effects: {forbidden}",
        )
    ...
    require(trade_call in monthly_owner, "Country trade owner must remain present")
    require(us04_call in monthly_owner, "Monthly US-04 owner must remain present")
    require(
    ...
    for metric_effect in (
        ...
        require(
            "cbp_observability_enabled_trigger" in block(live_effects, metric_effect),
            f"Observability guard missing from {metric_effect}",
        )
    ...
    for forbidden in (
        ...
        require(
            forbidden not in runtime_mermaid,
            f"Normative runtime diagram exposes retired owner surface: {forbidden}",
        )
    require(
        runtime_mermaid.count("every_market_in_world") == 1,
        "Normative runtime diagram must expose one every-market owner",
    )
